...
This code is wonderfully clean and concise and it sure would be nice if we could write our own functions that use the same syntax. And the good news is that we can! And the solution only relies on the use of the function `purrr::as_mapper()` which can convert a formula object into a function.
