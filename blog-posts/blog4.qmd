...
We can see what that binding actuall is by extracting it:

```{r}
fn_env(safe_mean)$`.f`
```

And there it is, the function environment has a binding between `.f` and the function `sum()`. This is cool, but to really understand why this happens we can make use of the `show_exec_env` argument in our `simple_safely` function

```{r}
safe_sum2 <- simple_safely(sum, show_exec_env = T)
```

This time we get to see the label for the execution environment of this instance of `simple_safely`. And if we then once again inspect the environment of our newly created function...

```{r}
env_print(safe_sum2)
```

we see that the enclosing environment of our `safe_sum2` function is the execution environment from the instance of `simple_safely` that created it! And it's this little fact that makes these function operations work. When `safe_sum2` looks for `.f` it is really looking in an execution environment of `simple_safely` which contains a binding for what that function should be!